onload = function () {
    ...
    var options = {
        edges: {
            width: 3,  // Set the width of the edges
    ...
    function createData() {
        ...
        cities = [ 'Warangal','Hyderabad', 'Jangoan', 'Vijayawada', 'Devarakonda', 'Madras', 'Delhi', 'Bangalore', 'Assam', 'Sikkim'];
        let nodes = [];
        for (let i = 1; i <= sz; i++) {
    ...
    genNew.onclick = function () {
        ...
        cnt++;
        if (cnt > 1) {
            temptext2.innerHTML = `Find least time path from ${cities[src - 1]} to ${cities[dst - 1]}<br>
                               *Yellow link represents bus travel time :<br>
                               *Green link represents flight travel time :<br>
        ...
        temptext2.innerHTML = `<span style="color: red;">Find least time path from ${cities[src - 1]} to ${cities[dst - 1]}</span>`;
        temptext.style.display = 'inline';
        temptext2.style.display = 'inline';
    };
